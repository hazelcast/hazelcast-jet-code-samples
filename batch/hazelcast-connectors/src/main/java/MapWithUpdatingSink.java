 ...
 * IMap updating sink with the Pipeline API.
 * This will take the contents of one map and write it into another map
 * using an update function.
 */
public class MapWithUpdatingSink {
            ...
            pipeline.drawFrom(Sources.map(SOURCE_MAP_NAME,
                    ...
                    .drainTo(
                            Sinks.mapWithUpdating(
                                    SINK_MAP_NAME,
                                    (oldValue, item) -> item.getValue() + "-updated"
                            )
                    );
