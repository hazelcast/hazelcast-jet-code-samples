 ...
 * We generate a stream of stock trade events and each event has
 * an associated product ID and broker ID. The reference list of products
 * and brokers are originally stored in files. The goal is to enrich the
 * trades with the actual name of the products and the brokers.
 * <p>
 ...
 * </ol>
 * <p>
 * The details of each approach is documented with the associated method.
 * <p>
 * The stream of trade events are generated by updating a single key
 * in the {@code trades} map which has the Event Journal enabled,
 * so that the Jet job receives
 * its update event stream.
 */
public final class Enrichment {
     ...
     * <p>
     * First, the files are read and cached into a {@code IMap} and then looked up
     * for every incoming trade using the
     * {@link StreamStage#mapUsingIMap(IMap, DistributedBiFunction)} method.
     * The list of products and brokers will be stored distributed in the cluster
     * so there might be additional lookup cost if the requested product or broker
     * is on a different node.
     * <p>
     * It is also possible to add new reference items or update them during job execution.
     */
     ...
     * <p>
     * Similarly to the {@code IMap} example, the files are read and
     * cached into a {@code ReplicatedMap} and then looked up
     * for every incoming trade using the
     * {@link StreamStage#mapUsingReplicatedMap(ReplicatedMap, DistributedBiFunction)} (IMap, DistributedBiFunction)}
     * method.
     * Unlike the {@code IMap} example, the list of products and brokers will be
     * replicated to every node so there will not be additional network cost for
     * lookup. The disadvantage is that more memory will be used as every node has
     * a whole copy of the data.
     * <p>
     * Likewise with {@code IMap}, it is also possible to add new reference items
     * or update them during job execution.
     */
    ...
    /**
     * Builds a pipeline which enriches the stream using
     * {@link GeneralStage#hashJoin(BatchStage, JoinClause, DistributedBiFunction)}.
     * <p>
     * Unlike the other examples which first read the data into a map, the reference
     * files in this case are read directly by the pipeline and then used as the
     * right sides of a hash-join operation.
     * <p>
     * With this approach, the full list will be held by every node locally
     * giving the fastest lookup time of all approaches. It also allows
     * the reference data to be read, pre-processed and then destroyed without
     * having to deal with the lifecycle of a separate {@code IMap} or {@code ReplicatedMap}.
     * <p>
     * The downside is that it's not possible to update the reference items or add new items
     * during runtime.
     */
