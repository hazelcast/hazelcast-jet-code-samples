...
public class ReadWriteRemoteList {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            Vertex source = dag.newVertex("source", Processors.readList(SOURCE_LIST_NAME, clientConfig))
                               .localParallelism(1);
            Vertex transform = dag.newVertex("transform", Processors.map((Integer i) -> Integer.toString(i)));
            Vertex sink = dag.newVertex("sink", Processors.writeList(SINK_LIST_NAME, clientConfig));
